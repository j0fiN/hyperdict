...
def test_class():
    assert inspect.isclass(hd.HyperDict) == True


def test_instance():
    d = hd.HyperDict()
    assert isinstance(d, hd.HyperDict) == True
...
def test_to_hd():
    ...
    assert str(d) == str(HyperDict({'language': 'Python', 'year': 1991, 'founded': 'Guido'}))
...
def test_values():
    assert list(hd_2.values()) == ['name', 2, 2.1, 1j, [2, 3, 1], {1: 0}, 'test_v']

def test_clear():
    assert hd_1.clear() == None
